        ...
        <form  method="POST" enctype="multipart/form-data" action="{{ route('posts.store') }}" >
            ...
            <div class="relative z-0 mb-6 w-full group ">
                <label for="title" class="block mb-2 text-sm font-medium text-slate-200 dark:text-gray-300">
                    ชื่อปัญหา (ปัญหาเล็กๆ เช่น ถังขยะหน้าห้องเรียนเต็มแล้ว หรือทํานําหก ให้ไปบอกกับแม่บ้านไม่ต้องกรอกลงในระบบ)
                </label>
                @if ($errors->has('title'))
            ...
            {{-- <div class="container box">
                ...
                <div class="from-group">
                    <select class="tag" name="tag" id="tag" class="from-control">
            ...
            <div class="col-md-12">
                           ...
                           @error('image')
                               <div class="alert alert-danger mt-1 mb-1 text-slate-200">{{ $message }}</div>
                           @enderror
                           </div>
            </div>
            <div>
                <button class=" app-button" type="submit">สร้าง</button>
                <a class="app-button text-slate-200" href="{{route('posts.incognito')}}">
                โหมดไม่ระบุตัวตน
                </a>
            </div>
        </form>
