    ...
    <section class="mx-8 font-mono bg-slate-800 rounded-lg px-4 py-6 pb-4 ring-1 ring-slate-900/5 shadow-xl">
        <h1 class="text-2xl mx-4 mt-6 pl-8 text-white">
            ...
            <form  method="get" action="{{route('posts.search')}}">
                <input type="text" name="search" id="search" placeholder="What do you want to find?" class="text-gray-900 text-sm">
            ...
            @foreach($posts as $post)
                <a href="{{ route('posts.show', ['post' => $post->id]) }}"
                   class="block p-6 w-full bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 ">
                    <h5 class="mb-4 text-xl font-bold tracking-tight">
                        {{ $post->title }}
                    ...
                    <img class="mb-4" src = "{{ asset('images/'.$post->image) }}"  alt="image-post" width=300 hieght=200 >
                    @if($post->status =='incognito')
                        <p class="pt-2 text-sm">
                            <p class="pt-2 text-sm">{{$post->created_at}}</p>
